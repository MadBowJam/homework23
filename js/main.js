...
function wordsList(str, subStr) {
    return [...new Set(str.replace(/[.,!?;:"']/g, '').toLowerCase().split(/\s+/).filter(word => word.includes(subStr)))];
}
...
let getLocalDate = (date, isSeconds = false, isISO = false) => {
    ...
    const format = isISO ? `${date.getFullYear()}-${padZero(date.getMonth() + 1)}-${padZero(date.getDate())}` : `${padZero(date.getDate())}.${padZero(date.getMonth() + 1)}.${date.getFullYear()}`;
    const time = `${padZero(date.getHours())}:${padZero(date.getMinutes())}${isSeconds ? `:${padZero(date.getSeconds())}` : ''}`;
...
let getLocalDay = (d) => (new Date(d).getDay() || 7);
...
function getDateAgo(dateString, days) {
...
const testPerformance = (iterations, func) => typeof func === 'function' ? (() => { let t = Date.now(); while (iterations--) func(); return Date.now() - t; })() : 0;
